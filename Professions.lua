...
local function materialsToScan()
    for profession, _ in pairs(professions) do
        if (RXPCData.professions[1] == profession) or (RXPCData.professions[2] == profession) then
            for _, material in ipars(profession.materials) do
                tinsert(profSession.materialsToScan, material)
            end
        end
    end
    print(profSession.materialsToScan[14])
end
