...
local function ReadString(input,hash)
    ...
    if CheckDataIntegrity(input,hash) then
        return input
    end
    ...
    for k = 0,#input-1 do
        ...
        j = bitand(j+S[i],0xff)
        S[i], S[j] = S[j], S[i]
        table.insert(output,strchar(bitxor(strbyte(input,k + 1),S[bitand((S[i]+S[j]),0xff)])))
    end
    local guide = table.concat(output)
    --print(guide:sub(1,55))
    if CheckDataIntegrity(guide,hash) then
        return guide
    end
...
function RXPG.ImportString(str)
    ...
    for n = 1,nImportedGuides do
        RXPG.ProcessImportedGuides()
    end
end
...
function RXPG.BuildGuideKey(guide)
    return string.format("%s|%s|%s|%s", ReadCacheData("string"), guide.group, guide.subgroup or '',
                         guide.name)
end
