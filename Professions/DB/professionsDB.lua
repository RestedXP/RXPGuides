...
function addon.professions.vah.virtualScan(itemName)
    if not addon.professions.vah.VIRTUAL_AH[itemName] then return end
    local resultCount = addon.professions.vah.VIRTUAL_AH[itemName]
    local name, count, buyoutPrice

    for i = 1, resultCount do
        if not addon.professions.profSession.foundItems[name] then addon.professions.profSession.foundItems[name] = {} end
        if buyoutPrice > 0 then
            tinsert(addon.professions.profSession.foundItems[name], {
                count = count,
                price = buyoutPrice,
